...
agency = 'lametro-rail'
datestring = '2018-12-31' # Can be the same date you last ran query_shedule.py
...
for line_no in range(801, 807):
    ...
    for path in trackpaths:
        ...
        with open(path) as data:
            track = LineString(json.load(data)['features'][0]['geometry']['coordinates'])
        ...
        line_stops = stops[stops['stop_id'].isin(stations)][['stop_id', 'stop_name', 'stop_lat', 'stop_lon']]
        gdf = gpd.GeoDataFrame(line_stops, geometry = [Point(xy) for xy in zip(line_stops.stop_lon, line_stops.stop_lat)])
        gdf = gdf.drop(['stop_lat', 'stop_lon'], axis=1)
        track_stations[name] = findRelativePositions(gdf, track)

# Blue line (801) has a different set of stations for each direction 
# due to a small loop.
# We remove them here:
edit = track_stations['801_northbound']
track_stations['801_northbound'] = edit[(edit['stop_id'].isin(['80153', '80154']) == False)]
...
for name in track_stations.keys():
    track_stations[name] = track_stations[name].sort_values('relative_position').reset_index().drop(['index'], axis=1)
